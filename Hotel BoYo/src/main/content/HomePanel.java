...
public class HomePanel extends JPanel implements MouseListener{
    ...
    {
        try {
            numGuest = getNumGuest();
        } catch (SQLException throwables) {
            throwables.printStackTrace();
        }
    }

    int noOfRoom = 50;//HARDCODED
    String numRoom;

    {
        try {
            numRoom = Integer.toString(noOfRoom - Integer.valueOf(getNumGuest()));
        } catch (SQLException throwables) {
            throwables.printStackTrace();
        }
    }

    private String getNumGuest() throws SQLException {
        String num="";
